...
Use the built-in backtest before tuning thresholds:

```bash
bun run backtest                 # last 8 days, $1,000 capital, 0.1% fee per side
bun run backtest --days=30       # longer window
bun run backtest --fee=0.0005    # custom fee/slippage assumption
```

The simulator compares strategy performance against buy & hold and applies the same stop-loss, take-profit, and trailing-stop guidance generated by the live signal engine.
...
bun run backtest  # simulate recent strategy performance
