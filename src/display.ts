...
export const render = (i: Indicators, s: SignalResult): string => [
  ...
  `${c.bold}${c.white}ATR Volatility${c.reset} ${i.volatility20.toFixed(3)}% hourly (${i.volatilityRatio.toFixed(2)}x baseline)`,
  `${c.bold}${c.white}Volume${c.reset}         ${i.volume20.toFixed(2)} BTC avg (${i.volumeRatio.toFixed(2)}x baseline)`,
